...
export default function SettingsPage() {
    ...
    const handleSelection = (item) => {
        ...
        // Protected items (notification, password)
        if (["notifications", "password"].includes(item.id) && !authUser) {
            toast.error("Please log in to success this setting");
            return;
        }
        setSelected(item.id);
    ...
    return (
        <div className="flex h-screen pt-15">
            ...
            >
                {/* Sidebar title */}
                <h2 className="text-3xl font-bold mb-3 tracking-wider myfont-kaushan">Settings</h2>
                            ...
                            return (
                                <li key={item.id}>
                                    <button
                                        ...
                                        disabled={disabled}
                                        className={`
                                            w-full flex items-center gap-3 text-left font-[Poppins] text-sm sm:text-lg
